...
func PruneStaleLinks(rootDir string, threshold float64) (PruneResult, error) {
	...
	return PruneResult{Removed: removed, Remaining: len(g.Edges)}, nil
}
...
// AddInterlinkedContext bulk-upserts a set of nodes and, when autoLink is true,
// automatically creates similarity edges between pairs whose content overlaps
// above a fixed threshold (0.72 cosine-approximated via token Jaccard).
func AddInterlinkedContext(rootDir string, items []struct {
	...
	result := &InterlinkResult{}

	for _, item := range items {
		n, err := UpsertNode(rootDir, item.Type, item.Label, item.Content, item.Metadata)
		if err != nil {
			return nil, err
		}
	...
	if !autoLink || len(result.Nodes) < 2 {
		return result, nil
	}
	...
	for i := 0; i < len(result.Nodes); i++ {
		for j := i + 1; j < len(result.Nodes); j++ {
			sim := jaccardSimilarity(result.Nodes[i].Content, result.Nodes[j].Content)
			if sim >= similarityThreshold {
				edge, err := CreateRelation(rootDir,
					result.Nodes[i].ID, result.Nodes[j].ID,
					RelationSimilarTo, sim, nil)
				if err != nil {
					return nil, err
				}
				if edge != nil {
