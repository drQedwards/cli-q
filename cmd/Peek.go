...
func Peek(opts PeekOptions) (*NodePeek, error) {
	...
	if opts.NodeID != "" {
		...
		labelLower := strings.ToLower(opts.Label)
		for i := range g.Nodes {
			if strings.ToLower(g.Nodes[i].Label) == labelLower {
				n := g.Nodes[i]
				target = &n
				break
			}
...
func PeekList(rootDir string) ([]NodePeek, error) {
	...
	for _, n := range g.Nodes {
		peeks = append(peeks, NodePeek{
			Node:     n,
			EdgesOut: edgesOut[n.ID],
			EdgesIn:  edgesIn[n.ID],
		})
	}
